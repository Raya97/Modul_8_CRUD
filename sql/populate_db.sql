...
) AS source (NAME, BIRTHDAY, LEVEL, SALARY)
...
WHEN NOT MATCHED THEN
    INSERT (NAME, BIRTHDAY, LEVEL, SALARY)
    VALUES (source.NAME, source.BIRTHDAY, source.LEVEL, source.SALARY);
...
MERGE INTO project_worker AS target
USING (
    VALUES
    (1, 1),
    (1, 2),
    ...
    (10, 9),
    (10, 10)
) AS source (PROJECT_ID, WORKER_ID)
ON target.PROJECT_ID = source.PROJECT_ID AND target.WORKER_ID = source.WORKER_ID
WHEN NOT MATCHED THEN
    INSERT (PROJECT_ID, WORKER_ID)
    VALUES (source.PROJECT_ID, source.WORKER_ID);
